...
# requirements-local-gui.txt sie bewusst nicht auflistet.
...
& $VenvPython -m pip install -r (Join-Path $ScriptDir "requirements-local-gui.txt")
if ($LASTEXITCODE -ne 0) {
    Write-Host "FEHLER: 'pip install -r requirements-local-gui.txt' ist fehlgeschlagen." -ForegroundColor Red
    exit 1
}
