...
#          Ollama-Modell qwen3:8b) -- Modelle-herunterladen.py
...
if (-not $Force -and (Test-PhaseDone "systempruefung")) {
    ...
    Push-Location $ScriptDir
    try {
        & $bootstrapPython (Join-Path $ScriptDir "Systempruefung.py")
...
if (-not $Force -and (Test-PhaseDone "modelle")) {
    ...
    Push-Location $ScriptDir
    try {
        & $VenvPython (Join-Path $ScriptDir "Modelle-herunterladen.py")
...
Push-Location $ScriptDir
try {
    & $VenvPython (Join-Path $ScriptDir "Systempruefung.py") --offline-check
