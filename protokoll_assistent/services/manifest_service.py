...
# Wie oft und wie lange ein misslungenes Ersetzen wiederholt wird.
ERSETZEN_VERSUCHE = 5
ERSETZEN_WARTEZEIT_SEKUNDEN = 0.1


def _ersetzen_mit_wiederholung(quelle: Path, ziel: Path) -> None:
    """Ersetzt 'ziel' durch 'quelle' und wiederholt kurz bei WinError 5.

    Unter Windows kann 'os.replace' mit "Zugriff verweigert" scheitern,
    obwohl mit den Rechten alles stimmt: Ein Virenscanner oder der
    Suchindex haelt die eben geschriebene Datei noch fuer Sekundenbruchteile
    offen. Ohne Wiederholung reisst das mitten in einer langen Verarbeitung
    den ganzen Lauf ab -- und zwar genau dann, wenn der Fortschritt gesichert
    werden soll. Der Fehler ist damit weder reproduzierbar noch erklaerbar;
    in der Testsuite ist er mehrfach zufaellig aufgeschlagen."""
    for versuch in range(ERSETZEN_VERSUCHE):
        try:
            quelle.replace(ziel)
            return
        except PermissionError:
            if versuch == ERSETZEN_VERSUCHE - 1:
                raise
            time.sleep(ERSETZEN_WARTEZEIT_SEKUNDEN)
...
def save_manifest(work_dir: Path, manifest: dict[str, Any]) -> None:
    ...
    _ersetzen_mit_wiederholung(Path(temp_name), path)
