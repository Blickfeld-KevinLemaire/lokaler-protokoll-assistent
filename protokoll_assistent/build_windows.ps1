...
# Die Spezifikation liegt in der Projektwurzel, weil sie das Paket baut.
Push-Location (Split-Path -Parent $ScriptDir)
try {
    & $VenvPython -m PyInstaller --noconfirm protokoll_assistent.spec
    if ($LASTEXITCODE -ne 0) {
...
$DistDir = Join-Path (Split-Path -Parent $ScriptDir) "dist\Protokoll-Assistent"
...
$ExePath = Join-Path (Split-Path -Parent $ScriptDir) "dist\Protokoll-Assistent\Protokoll-Assistent.exe"
