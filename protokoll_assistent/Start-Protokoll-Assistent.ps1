...
# 'protokoll_assistent.app'. Die Anwendung selbst kuemmert sich beim ersten Start
# um alles Weitere:
...
Write-Host "=== Protokoll-Assistent ===" -ForegroundColor Cyan
...
# Aus der Projektwurzel heraus und als Modul starten - nur so ist das
# Paket 'protokoll_assistent' importierbar.
Push-Location (Split-Path -Parent $ScriptDir)
try {
    & $pythonExe @pythonArgs -m protokoll_assistent.app
