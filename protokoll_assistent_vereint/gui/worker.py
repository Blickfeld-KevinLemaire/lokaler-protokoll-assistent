...
Fast wortgleich mit 'lokale_windows_app/gui/worker.py', mit einer
Erweiterung: die Worker nehmen zusaetzlich optionale
...
implementierungen) als auch mit den neuen API-Funktionen aus
...
# Bewusst der bare Modulname 'services', nicht 'protokoll_assistent_vereint.services':
# 'pipeline_service' und seine gesamte Ablaufsteuerung leben unveraendert in
# 'lokale_windows_app/services/' und werden wiederverwendet, nicht kopiert
# (siehe 'app.py' zum Eintragen von 'lokale_windows_app' in sys.path).
...
class DateiHashWorker(QThread):
    ...
    'lokale_windows_app/gui/worker.py' fuer die Begruendung: der Hash
