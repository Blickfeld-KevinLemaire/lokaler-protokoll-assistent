"""Hauptfenster der vereinten Anwendung (PySide6).
...
``services.pipeline_service`` aus ``lokale_windows_app`` - unveraendert
wiederverwendet, nicht kopiert (siehe ``app.py``). Ob ein Chunk lokal oder
ueber eine API transkribiert wird, ist dort bereits nur eine austauschbare
Funktion (``transcribe_chunk_fn``/``diarize_fn``/``protocol_generate_fn``) -
diese Datei waehlt lediglich, welche Funktion(en) das sind.
"""
...
# Wiederverwendete Module aus 'lokale_windows_app' (bare, siehe 'app.py' -
# dort wird 'lokale_windows_app' in sys.path eingetragen). 'get_work_dir'
# und 'get_system_prompt_file' MUESSEN von hier kommen, nicht aus dem
# eigenen 'utils.paths': 'services.pipeline_service' loest beide intern
# genauso bare auf - ein eigener Satz dieser beiden Funktionen wuerde
# schlicht nie gelesen (siehe Docstring von 'utils/paths.py').
...
from protokoll_assistent_vereint.utils.systemprompt_vorlagen import alle_vorlagen, vorlage_speichern
